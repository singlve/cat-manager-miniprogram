...
Page(Object.assign({}, catFormMethods, {
  ...
  onLoad(options) {
    ...
    const species = options && (options.species === "cat" || options.species === "dog")
      ? options.species
      : "";
    if (species) {
      this.speciesChange({ detail: { value: species } });
      this.setData({ speciesLocked: String(options.lockSpecies || "") === "1" });
    }
  },
  ...
  onShareAppMessage() {
    return { imageUrl: '/assets/logo.jpg', title: '为宠物建立一份专属成长档案', path: '/pages/cat-list/cat-list' };
  },
}));
