...
Page({
  ...
  goWeightRecords() {
    const app = getApp();
    if (!app.isLoggedIn()) { this._promptLogin(); return; }
    wx.navigateTo({ url: '/pet-package/weight-records/weight-records?catId=' + this.data.catId });
  },
  ...
  goHealthRecords() {
    const app = getApp();
    if (!app.isLoggedIn()) { this._promptLogin(); return; }
    wx.navigateTo({ url: '/pet-package/health-records/health-records?catId=' + this.data.catId });
  },
  ...
  goCareReminder(e) {
    ...
    wx.navigateTo({ url: `/pet-package/reminder-add/reminder-add?${params.join('&')}` });
  },

  goEdit() {
    const app = getApp();
    if (!app.isLoggedIn()) { this._promptLogin(); return; }
    wx.navigateTo({ url: '/pet-package/cat-edit/cat-edit?id=' + this.data.catId });
  },
  ...
  onShareAppMessage() {
    ...
    return {
      imageUrl: this.data.shareImagePath || '/assets/logo.jpg',
      title: `看看${name}的健康档案`,
      path: `/pet-package/cat-detail/cat-detail?id=${this.data.cat?._id || this.data.catId || ''}`
    };
  }
});
