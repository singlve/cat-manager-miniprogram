...
Page({
  data: {
    ...
    showEditor: false,
    editingId: null,
    form: { name: '', type: 'virtual', virtualType: 'card', virtualValue: 1, points: 0, stock: 9999, desc: '', image: '', enabled: true, isSystemTheme: false },
    saving: false,
    // 发货弹窗
  ...
  async loadAll() {
    ...
    try {
      await clouddb.ensureThemeRedeemItems();
    } catch (e) {
      console.error('[admin-items] ensure theme products fail:', e);
    }
    ...
    if (primaryResults.every(function(result) { return result.status === 'rejected'; })) {
      console.error('[admin-items] all primary data sources failed');
      this.setData({
        items: [],
        records: [],
        rawRecords: [],
  ...
  openEditItem(e) {
    ...
    this.setData({
      ...
      form: {
        ...
        isSystemTheme: item.virtualType === 'theme'
      }
    });
  },
  ...
  async deleteItem(e) {
    ...
    var item = (this.data.items || []).find(function(current) { return current._id === id; });
    if (item && item.virtualType === 'theme') {
      wx.showToast({ title: '主题商品请使用下架操作', icon: 'none' });
      return;
    }
  ...
  async saveForm() {
    ...
    try {
      ...
      if (form.type === 'virtual') {
        ...
        itemData.virtualValue = form.virtualType === 'theme'
          ? String(form.virtualValue)
          : (parseInt(form.virtualValue) || 1);
        if (form.virtualType === 'theme') {
          itemData.stock = 9999;
          itemData.systemManaged = true;
          itemData.systemKey = 'theme:' + form.virtualValue;
        }
      }
      if (editingId) {
