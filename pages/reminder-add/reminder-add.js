...
Page({
  ...
  intervalInput(e) {
    this.setData({ intervalDays: Math.min(365, Math.max(1, parseInt(e.detail.value) || 30)) });
    this.updateNextPreview();
  },

  setInterval(e) {
    this.setData({ intervalDays: parseInt(e.currentTarget.dataset.days) });
    this.updateNextPreview();
  },
  ...
  async save() {
    const { isEdit, reminderId, selectedCatId, selectedCatName, type, lastDate, intervalDays, note } = this.data;
